...
- **Detect pauses from the model, not from the shipped forced alignment.** Tadabur's
  ...
  timestamps). Instead, decode the whole clip with the Muaalem **phoneme head**: greedy CTC emits
  the **blank** token on silence, so a run of blank frames is a pause the model itself heard —
  aligner-independent and immune to madd (a madd is voiced and decodes to real phonemes). A
  blank-run ≥ **0.35 s** (`--min-pause`) is a waqf candidate; shorter runs are inter-phoneme /
  stop-consonant closures.

- **Map each pause to a *word* boundary via Smith-Waterman; self-reject mid-word stops.** A blank-
  run gives a *time*, but a segment must cut on a word so it can be phonetized. The decoded
  phonemes are Smith-Waterman-aligned to the ayah's per-word reference, and each run is placed at
  the reference position of the last phoneme before it. A run that lands within **3 phonemes**
  (`--boundary-tol`) of a word edge is a waqf; one that lands mid-word is a stop-consonant closure
  (qalqala on ق/ط, the hamza in شَيء) and is **not** split. This Δ-to-edge test is what cleanly
  separates a real waqf (Δ≈0) from a mid-word closure (Δ≫tol) — the discriminator the acoustic
  gate lacked.
...
- **This stage owns the model (GPU).** Segmentation + per-segment scoring live in
  `tadabur.segment_score`; each clip is decoded **one at a time** (a batched decode of full clips
  ...
  sequence plus the reference/boundaries, so it is unit-tested with synthetic `class_ids` and no
  model.
...
- **One model pass, decoded twice.** The whole clip is decoded once for segmentation (frame ids)
  and each resulting segment is decoded again for scoring against its realized reference — the same
  `.balanced` normalization / Smith-Waterman / contrast attribution the full-ayah filter uses. The
  second decode is what the audit samples on; it is observational (no gate-reject).
